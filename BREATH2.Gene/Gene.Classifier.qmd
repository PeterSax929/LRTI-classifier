...
# Comment below for CV lasso
  ...
  if (any(nz == 1)) {
    lambda_k1 <- fit$lambda[which(nz == 1)[1]]   # largest lambda w/ 3 nonzero
  } else {
...
pred_vec <- predict(fit, newx = X_test, type = "response", s = lambda_k1)[, 1]
...
preds <- data.frame(
  ...
  coefs_vec <- coef(fit, s = lambda_k1)[-1, 1]
...
# Comment below for CV lasso  
  ...
  # Ensure at least 3 genes per fold (like protein workflow)
if (length(nonzero) < 1) {
  # Find all lambdas that select >= 3 nonzero genes
  lambda_idx_candidates <- which(nz >= 1)
  if (length(lambda_idx_candidates) == 0) {
    # If no lambda has >=3, pick lambda with closest >3 
    idx <- which.min(abs(nz - 1))
    lambda_k1 <- fit$lambda[idx]
  } else {
    # Pick the largest lambda that yields at least 3 nonzeros
    lambda_k1 <- fit$lambda[lambda_idx_candidates[1]]
  }
