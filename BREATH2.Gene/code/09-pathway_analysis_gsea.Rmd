...
plot_df <- hallmark_to_plot %>%
  ...
  gsub("_", " ",
       gsub("^HALLMARK_", "", Description)), width = 40))
...
p_compare <- ggplot(
  ...
  labs(
    ...
    subtitle = expression("Top pathways ranked by combined |NES| — Filled = P"[adj]*" < 0.05"),
    x = "Normalized Enrichment Score (NES)",
    y = NULL,
    color = "Omics layer"
  ) +
...
# Save plot
ggsave(
  filename    = file.path(results_dir, "GSEA_Hallmark_RNA_vs_Protein.pdf"),
  plot        = p_compare,
  width       = 8,
  height      = 7,
  device      = "pdf",
