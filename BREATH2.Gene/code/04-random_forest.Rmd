...
#selected_lasso_coefs <- data.frame(
  #gene_symbol = rep("ISG15", max(metadata_df_with_folds$fold)),
  #gene        = rep("ENSG00000187608", max(metadata_df_with_folds$fold)),
  #coef        = NA,
  #fold        = 1:max(metadata_df_with_folds$fold)
 #)
...
# 4.2a. Use precomputed vst from LASSO
  ...
  X_all <- X_all[metadata_df_with_folds$sample_name, , drop=F]
...
# Save RF predictions (change file to target lambda)
write.csv(rf_preds, 
...
all(colnames(full_vsd) == metadata_df_with_folds$sample_name) # Should return TRUE
