...
# Set to NULL for lambda.1se, or 1 or 3 for fixed-n
target_n <- NULL
...
# 3.2d. Select target lambda
  ...
  if (is.null(target_n)) {
    ...
    pred_vec <- predict(
      fit, newx = X_test, type = "response", s = cv_fit$lambda.1se)[, 1]
    ...
    pred_vec <- predict(fit, newx=X_test, type="response",
                        s=lambda_target)[, 1]
  }
...
# 3.2e. Ensure structure and build data frame 
...
coef_filename <- if (is.null(target_n)) "lasso_fold_coefs.csv" else
                 paste0("lasso_fold_coefs_n", target_n, ".csv")

write.csv(lasso_fold_coefs, file.path(results_dir, coef_filename), row.names=F)
