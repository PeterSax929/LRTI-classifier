...
merged_df <- merged_df %>%
  ...
  mutate(
    point_size = case_when(
      sig_class == "RNA & Protein" ~ 2.5,
      sig_class == "RNA only"      ~ 1.4,
      sig_class == "Protein only"  ~ 1.4,
      TRUE                         ~ 0.8
    ))
...
label_df <- merged_df %>%
  ...
  bind_rows(
    merged_df %>%
      filter(sig_class == "RNA & Protein") %>%
      mutate(concordance = sign(log2FC_RNA) == sign(log2FC_protein),
...
ggplot(
  ...
  geom_point(aes(size = point_size),
    alpha = ifelse(merged_df$sig_class == "Not Significant", 0.3, 0.75)
    ) +
  scale_size_identity() +
  geom_smooth(
  ...
  subtitle = paste0(
    "All features: Spearman p = ", round(cor_spearman, 2),
    " | Jointly significant: p = ",
    round(cor_spearman_both, 2),
    " (n = ", sum(merged_df$sig_class == "RNA & Protein"), ")"
  ),
  color = "Significance"
  ) +
  theme(
...
ggsave(
  file.path(results_dir, "RNA_protein_LFC_concordance.pdf"),
  width       = 7,
  height      = 6,
  device      = "pdf",
  useDingbats = FALSE
