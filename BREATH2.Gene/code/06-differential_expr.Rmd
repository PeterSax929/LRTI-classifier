...
ggplot(res_df, aes(x=log2FoldChange, y=negLog10P)) +
  geom_point(aes(color=direction), alpha=0.65, size=1.6) +
  scale_color_manual(
    values=c(
  "Definite"="red",
  "NoEvidence"="blue",
  "NotSignificant"="grey80"
    ),
  ...
  geom_hline(
    yintercept=-log10(0.05), linetype="dashed", linewidth=0.6, color="grey40"
    ) +
  ...
  labs(
    ...
    caption=
      paste0("Differentially expressed genes (FDR < 0.05): ", sum(res_df$sig))
  ) +
