...
cor_ifn_df <- ifn_combined_df %>%
  ...
  summarize(
    pearson_r = cor.test(expr_scaled, log10(viral_rpm + 1),
...
ggplot(
  ...
  aes(
    ...
    y = y_pos,
    label = paste0(modality, ": r = ", round(pearson_r, 2))
  ),
  inherit.aes = FALSE,
  size = 3,
  hjust = 1
  ...
  aes(
    x = max(log10(ifn_combined_df$viral_rpm + 1)) * 0.98,
    y = 3.1,
    label = paste0("p value = ", round(interaction_p, 3))
  ),
  inherit.aes = FALSE,
  size = 3,
  hjust = 1
...
cor_composite_df <- ifn_score_combined_df %>%
  group_by(modality) %>%
  summarize(
    pearson_r = cor.test(IFN_score, log10(viral_rpm + 1),
      method = "pearson")$estimate,
    .groups = "drop"
  )
...
ggplot(
  ...
  x = max(log10(ifn_score_combined_df$viral_rpm + 1)) * 0.98,
  y = -1.7,
  label = paste0("p value = ", signif(interaction_p_composite, 3)),
  size = 4,
  hjust = 1
  ) +
...
plot_ifn_score <- function(score_df, ifn_label, filename) {
  ...
  cor_df <- score_df %>%
    ...
    mutate(y_pos = case_when(
      modality == "RNA"     ~ min(score_df$IFN_score, na.rm=T) + 0.3,
      modality == "Protein" ~ min(score_df$IFN_score, na.rm=T) + 0.1
    ))
  ...
  p <- ggplot(score_df, aes(x=log10(viral_rpm + 1), y=IFN_score, color=modality)) +
    ...
    geom_text(
      data = cor_df,
      aes(x=max(log10(score_df$viral_rpm + 1)) * 0.98, y=y_pos,
          label=paste0(modality, ": r = ", round(pearson_r, 2))),
      inherit.aes=FALSE, size=4, hjust=1
    ) +
    annotate("text",
      x = max(log10(score_df$viral_rpm + 1)) * 0.98,
      y = min(score_df$IFN_score, na.rm=T) - 0.1,
      label = paste0("Interaction p = ", signif(interaction_p, 3)),
      size=4, hjust=1
    ) +
    scale_color_manual(values=omics_colors) +
    theme_classic() +
