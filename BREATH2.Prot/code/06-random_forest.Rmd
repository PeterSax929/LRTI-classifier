...
# Single biomarker override — change protein name here
#selected_lasso_coefs <- data.frame(
  #protein = rep("ISG15", max(patient_info_with_folds$fold)),
  #coef    = NA,
  #fold    = 1:max(patient_info_with_folds$fold)
#)
...
rf_preds <- cv_list %>%
  lapply(function(x) {
    ...
    rownames(protein_mat)[patient_info_with_folds$fold == x$test_fold],
    pred = x$pred
    )
  }) %>%
  bind_rows() %>%
...
# Save RF predictions (change csv file output depending on lambda used)
write.csv(rf_preds, 
...
all(rownames(protein_mat) == patient_info_with_folds$patient_id) # Should return TRUE
