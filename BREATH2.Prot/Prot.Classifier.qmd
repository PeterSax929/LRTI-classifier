...
# Fallback if no lambda selects 1 protein
#if (length(nonzero) == 0) {
  #best_name <- names(coefs_vec)[which.max(abs(coefs_vec))]
  #nonzero <- coefs_vec[best_name]
#}
...
#--------------------Select lambda based on LASSO CV---------------------------#
...
# Import protein matrix (rows = patients, cols = proteins)
protein_mat <- read.csv(
  file.path(data_dir, "protein_mat.csv"),
  stringsAsFactors = FALSE,
  row.names = 1,
...
patient_info_with_folds$patient_id <- 
  as.character(patient_info_with_folds$patient_id)




#-----------------------8b. Prepare matrix for PCA-----------------------------#



# Matrix already samples x proteins
X_pca <- as.matrix(protein_mat)
...
#-----------------------8d. Build PCA scores dataframe-------------------------#



scores_df <- as.data.frame(pca_res$x) %>%
  tibble::rownames_to_column("patient_id") %>%
  left_join(patient_info_with_folds, by = "patient_id")



#-----------------------8e. Variance explained---------------------------------#
...
ggplot(
  scores_df,
  aes(x = PC1, y = PC2, color = LRTI_adjudication)
) +
  geom_point(size = 3, alpha = 0.8) +
  labs(
    title = "PCA Score Plot (Proteomics)",
...
#-----------------------8g. Optional: color by fold----------------------------#



 ggplot(
   scores_df,
   aes(x = PC1, y = PC2, color = factor(fold))
 ) +
   geom_point(size = 3, alpha = 0.8) +
   labs(
     title = "PCA Score Plot Colored by CV Fold",
     x = paste0("PC1 (", pc1_var, "% variance)"),
     y = paste0("PC2 (", pc2_var, "% variance)")
   ) +
   theme_minimal()
 ...
 #--------------------------8h. Correlation with PC1---------------------------#

# PC1 scores
pc1_scores <- pca_res$x[, 1]

# Build combined dataframe
protein_df <- protein_mat %>%
  as.data.frame() %>%
  rownames_to_column("patient_id")

clinical_df <- patient_info_with_folds %>%
  transmute(
...
full_df <- left_join(protein_df, clinical_df, by = "patient_id")

X_corr <- full_df %>% select(-patient_id)

# Explicit correlation dataframe (no sapply pitfalls)
cor_df <- tibble(
  variable = colnames(X_corr),
  correlation = apply(
    X_corr,
    2,
    function(v) cor(v, pc1_scores, use = "pairwise.complete.obs")
  )
) %>%
  arrange(correlation)
...
#--------------------------8f. Diverging correlation bar plot-----------------#
...
ggplot(
  ...
  labs(
    title = "Variable correlations with PC1",
